(function ($, Drupal) {
  ...
  /**
   * Provide vertical tab summaries for Invent settings.
   */
  Drupal.behaviors.inventSettingSummaries = {
    attach: function (context) {
      ...
      $context.find('#edit-components').drupalSetSummary(function () {
        var summary = [];
        // Breadcrumbs.
        var breadcrumb = parseInt($context.find('select[name="invent_breadcrumb"]').val(), 10);
        if (breadcrumb) {
          summary.push(Drupal.t('Breadcrumbs'));
        }
        // Navbar.
        var navbar = 'Navbar: ' + $context.find('select[name="invent_navbar_position"] :selected').text();
        if ($context.find('input[name="invent_navbar_inverse"]').is(':checked')) {
      ...
      $context.find('#edit-javascript').drupalSetSummary(function () {
        var summary = [];
        if ($context.find('input[name="invent_anchors_fix"]').is(':checked')) {
          summary.push(Drupal.t('Anchors'));
        }
        if ($context.find('input[name="invent_popover_enabled"]').is(':checked')) {
          summary.push(Drupal.t('Popovers'));
        }
        if ($context.find('input[name="invent_tooltip_enabled"]').is(':checked')) {
      ...
      $context.find('#edit-advanced').drupalSetSummary(function () {
        var summary = [];
        // InventCDN.
        var inventCDN = $context.find('select[name="invent_cdn"]').val();
        if (inventCDN.length) {
          inventCDN = 'InventCDN v' + inventCDN;
          // Bootswatch.
          if ($context.find('select[name="invent_bootswatch"]').val().length) {
            inventCDN += ' (' + $context.find('select[name="invent_bootswatch"] :selected').text() + ')';
          }
          summary.push(inventCDN);
        }
        // Rebuild registry.
        if ($context.find('input[name="invent_rebuild_registry"]').is(':checked')) {
  ...
  /**
   * Provide Invent Bootswatch preview.
   */
  Drupal.behaviors.inventBootswatchPreview = {
    attach: function (context) {
      var $context = $(context);
      ...
      $preview.once('bootswatch', function () {
        $.get("http://api.bootswatch.com/3/", function (data) {
          ...
          $preview.parent().find('select[name="invent_bootswatch"]').bind('change', function () {
  ...
  /**
   * Provide Invent navbar preview.
   */
  Drupal.behaviors.inventNavbarPreview = {
    attach: function (context) {
      var $context = $(context);
      ...
      $preview.once('navbar', function () {
        ...
        $preview.find('select[name="invent_navbar_position"]').bind('change', function () {
        ...
        $preview.find('input[name="invent_navbar_inverse"]').bind('change', function () {
